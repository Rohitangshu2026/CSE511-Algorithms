...
int goodPivot(vector<int>& nums, int left, int right) 
{
    ...
    // Use (groups + 1) / 2 for correct median rank 
...
int findElement(vector<int>& nums,int left,int right,int rank)
{
    vector<int> buffer(2*rank);

    // filling the right half of the buffer
    for(int i = 0; i < rank; ++i)
        buffer[i+rank] = nums[i];

        int val;
    // filling the left half of the buffer and finding median
    for(int i = rank; i <= right; i += rank)
    {
        int j = 0;
        while(j < rank && j + i <= right)
        {
            buffer[j] = nums[j+i];
            j++;
        }
        val = findElementWithRank(buffer,0,2*rank - 1,rank);
    }
    return buffer[val];
...
int main()
{
    ...
    int val = findElement(nums, 0, n - 1, rank);
    cout << "Element with rank " << rank << " is: " << val << endl;
    return 0;
}
